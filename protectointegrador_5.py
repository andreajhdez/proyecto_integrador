...
class JuegoArchivo:
    ...
    # Se define el método para leer archivos .txt en la carpeta y sacar el mapa y los puntos de inicio y de fin.
    def leer_archivos(self, ruta):
        # De acá encontramos la lista de archivos sobre los que elegiremos el mapa
        dir_list = os.listdir(ruta)
        # Con random elegimos un mapa aleatorio.
        mapa_random = random.choice(dir_list)
        # Esta es la ruta completa junto con el nombre del mapa
        path_completo = f"{ruta}/{mapa_random}"
        # Se abre el archivo se lee y se extrae su contenido.
        with open(path_completo, 'r') as archivo:
            ...
            p_i = (coord[1], coord[0])
            p_f = (coord[3], coord[2])
        # Se retorna matriz de caracteres para el mapa y los puntos iniciales y finales
